				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((value >= sizeof(ia6xx_bypass_mode_values)/sizeof(ia6xx_bypass_mode_values[0]))){
		pr_err("value: %d is not a valid value", value);
		return rc;
	}
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value != IA_ROUTE_SELECTED_NONE &&
			(value < sizeof(ia6xx_select_route_values)/sizeof(ia6xx_select_route_values[0]))){
...
static struct kobj_type iacore_priv_ktype = {
	...
	defined(CONFIG_SND_SOC_SDM845) || defined(CONFIG_ARCH_SDM670))
...
int ia6xx_core_probe(struct device *dev)
{
	...
	init_completion(&iacore->snd_reg);
