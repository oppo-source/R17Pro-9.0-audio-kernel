...
#else
	cmd = IA_ALGO_RESET << 16 | IA_OK_GOOGLE_ALGO_ID;
	rc = iacore_cmd_nopm(iacore, cmd, &rsp);
	if (rc) {
		pr_err("Reset Algo command fail %d\n", rc);
		goto vs_algo_param_err;
	}
#endif
	}
...
void iacore_notify_fw_download(void)
{
	struct iacore_priv *iacore = iacore_global;
	if (iacore) {
		complete(&iacore->snd_reg);
		pr_info("sound card is registered!\n");
	}
}
EXPORT_SYMBOL(iacore_notify_fw_download);

void iacore_request_fw_cb(const struct firmware *fw, void *context)
{
	struct iacore_priv *iacore = context;
	struct iacore_voice_sense *voice_sense = GET_VS_PRIV(iacore);
#if defined(CONFIG_SND_SOC_IA_OK_GOOGLE)
	const char *hw_filename = "audience/ia6xx/ia6xx-okg-hw.bin";
#else
...
#if defined(CONFIG_SND_SOC_IA_OK_GOOGLE)
	const char *bargein_hw_filename = "audience/ia6xx/ia6xx-okg-bargein-hw.bin";
#else
	...
	//do hw reset
	rc = iacore_change_state_lock_safe(iacore, POWER_OFF);
	if (rc) {
		pr_err("set to power off failed rc = %d\n", rc);
	}

skip_barge_in_fw_check:

	if (!wait_for_completion_timeout(&iacore->snd_reg,
				msecs_to_jiffies(60 * 1000))) {
		pr_err("wait for snd card register timeout\n");
	}
...
#if	!defined(CONFIG_SND_SOC_IA_OK_GOOGLE)
		pr_crit("%s Again i am not having google config\n", __func__);
...
long ia_cvq_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	u32 kw_length = 0;
	u32 cmd_to_send;
//#ifdef CONFIG_SND_SOC_IA_OK_GOOGLE
//	u32 command, rsp = 0;
//#endif
	...
	switch (cmd) {
	case IA_INIT_PARAMS:
		pr_debug("initialize CVQ params\n");
	...
	case IA_INIT_IOSTREAM_PARAMS:
		pr_debug("initialize iostream params\n");
		if (copy_from_user(&params, (void __user *)arg,
				   sizeof(struct ia_cvq_params))) {
			ret = -EFAULT;
			goto err_exit;
	...
	case IA_CVQ_START:
...
#if	defined(FW_SLEEP_TIMER_TEST)
		...
		pr_info("start CVQ barge_in_required = %d\n", barge_in_required);
	...
	case IA_GET_KW_LENGTH:
		dev_info(iacore->dev, "%s get KW Length\n", __func__);

		/* Send KW Length read command */
		cmd_to_send = IA_GET_KW_LENGTH_CMD << 16 | IA_KW_LEN_ALGO_PARAM;

		ret = iacore_cmd_nopm(iacore, cmd_to_send, &kw_length);
		if (ret) {
			pr_err("Get KW Length cmd fail %d", ret);
			kw_length = 0;
		}

		kw_length &= 0x0000FFFF;
		pr_info("kw_length = %d", kw_length);

		if (copy_to_user((void *)arg, &kw_length, sizeof(u32))) {
			pr_err("copy_to_user get kw length failed");
			ret = -EFAULT;
		}
		break;

	case IA_UNLOAD_KEYWORDS:
		pr_debug("flush keyword files\n");
		iacore_release_models_unlocked(iacore);
//#ifdef CONFIG_SND_SOC_IA_OK_GOOGLE
#if 0
		/*
		 * At the end of training, HAL/Upper layer will 1st call flush
		 * keyword models & then stops recording. Chip will be in low
		 * power capture route (which puts the chip in sleep).
		 * 1st wakeup the chip & then set the Algo reset command to
		 * flush-out old Keywords.
		 */
		ret = iacore_wakeup_unlocked(iacore);
		if (ret) {
			dev_warn(iacore->dev, "%s() wakeup fail %d\n",
								__func__, ret);
			break;
		}
		...
		/*
		 * when we wake the chip from low latency route, it breaks the
		 * route in the chip. Clear the route flags after wakeup in sw.
		 */
		if (iacore->low_latency_route_set == true)
			iacore->low_latency_route_set = false;

		if (iacore->active_route_set == true)
			iacore->active_route_set = false;

		command = IA_ALGO_RESET << 16 | IA_OK_GOOGLE_ALGO_ID;
		ret = iacore_cmd_nopm(iacore, command, &rsp);
		if (ret)
			dev_err(iacore->dev,
				"%s() Reset Algo cmd fail %d\n", __func__, ret);
#endif
		break;
	case IA_GET_KW_ID:
		pr_debug("get keyword id\n");
	...
	case IA_LOAD_FIRMWARE:
		pr_debug("Load Firmware to fw_sleep\n");
#ifdef CONFIG_SND_SOC_IA_I2S_PERF
		ret = iacore_change_state_unlocked(iacore, FW_SLEEP);
		if (ret < 0)
	...
	case IA_SETUP_BARGEIN:
		...
		if (ret < 0) {
			...
			pr_debug("clear potential pending irq, set irq_event_detected as false\n");
			iacore->irq_event_detected = false;
		}
		break;
#endif
