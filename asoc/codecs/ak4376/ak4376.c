...
struct ak4376_priv {
	...
	#endif /* VENDOR_EDIT */
	#ifdef VENDOR_EDIT
	/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/08/03, Add for usb type-c audio */
...
#ifdef VENDOR_EDIT
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/07/16, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
static struct snd_soc_codec *ak4376_codec_info;
#endif
#endif /* VENDOR_EDIT */
...
extern int snd_soc_cache_init(struct snd_soc_codec *codec);
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/08/03, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
static int ak4376_pdn_control_non_blocked(struct snd_soc_codec *codec, int pdn)
{
    struct ak4376_priv *ak4376 = snd_soc_codec_get_drvdata(codec);
    int ret = 0;
...
#ifdef USB_SWITCH_MAX20328
/* HP-AMP */
static int ak4376_HPAmpL_event2(struct snd_soc_codec *codec, int event)
{
    struct ak4376_priv *ak4376 = snd_soc_codec_get_drvdata(codec);
    pr_info("%s event = %d\n", __func__, event);
    switch (event) {
    case SND_SOC_DAPM_PRE_PMU:    /* before widget power up */
        //snd_soc_update_bits(codec, AK4376_04_OUTPUT_MODE_SETTING, 0x01, 0x00);
        mdelay(6);                                                                //wait 6ms                                                              //wait 1ms
        break;
    case SND_SOC_DAPM_POST_PMU:    /* after widget power up */                                                           //wait 0.5ms
        break;
    case SND_SOC_DAPM_PRE_PMD:    /* before widget power down */
        mutex_lock(&ak4376->pdn_lock);
        //snd_soc_update_bits(codec, AK4376_04_OUTPUT_MODE_SETTING, 0x01, 0x01);
        mdelay(6);                                                                //wait 6ms
        mutex_unlock(&ak4376->pdn_lock);
        break;
    case SND_SOC_DAPM_POST_PMD:    /* after widget power down */
        ...
        struct snd_kcontrol *kcontrol, int event) //CONFIG_LINF
{
    ...
    ak4376_HPAmpL_event2(codec, event);

    return 0;
}
static int ak4376_HPAmpR_event2(struct snd_soc_codec *codec, int event)
{
    struct ak4376_priv *ak4376 = snd_soc_codec_get_drvdata(codec);
	pr_info("%s event = %d\n", __func__, event);
    switch (event) {
    case SND_SOC_DAPM_PRE_PMU:    /* before widget power up */
        //snd_soc_update_bits(codec, AK4376_04_OUTPUT_MODE_SETTING, 0x02, 0x00);
        mdelay(6);                                                                //wait 6ms                                                              //wait 1ms
        break;
    case SND_SOC_DAPM_POST_PMU:    /* after widget power up */                                                           //wait 0.5ms
        break;
    case SND_SOC_DAPM_PRE_PMD:    /* before widget power down */
        mutex_lock(&ak4376->pdn_lock);
        //snd_soc_update_bits(codec, AK4376_04_OUTPUT_MODE_SETTING, 0x02, 0x02);
        mdelay(6);                                                                //wait 6ms
        mutex_unlock(&ak4376->pdn_lock);
        break;
    case SND_SOC_DAPM_POST_PMD:    /* after widget power down */
        ...
        struct snd_kcontrol *kcontrol, int event) //CONFIG_LINF
{
    ...
    ak4376_HPAmpR_event2(codec, event);
...
int ak4376_HPLR_HZ(bool HPLR_HZ)
{
    ...
    if(HPLR_HZ) {
        ...
        if (ret & 0x3) {
            ...
            snd_soc_update_bits(ak4376_codec_info, AK4376_03_POWER_MANAGEMENT4, 0x03, 0x00);
            msleep(30);
        } else {
            ak4376->amp_pending = false;
        }
        //setup pull-down resistor
        snd_soc_update_bits(ak4376_codec_info, AK4376_04_OUTPUT_MODE_SETTING, 0x03, 0x03);
        msleep(20);
    } else {
        //setup pull-down resistor
        snd_soc_update_bits(ak4376_codec_info, AK4376_04_OUTPUT_MODE_SETTING, 0x03, 0x00);
        msleep(20);
        if (ak4376->amp_pending) {
            pr_info("%s: recover amp\n", __func__);
            ak4376->amp_pending = false;
            snd_soc_update_bits(ak4376_codec_info, AK4376_03_POWER_MANAGEMENT4, 0x03, 0x03);
            msleep(30);
        }
        //release PDN
...
static int ak4376_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
    ...
    pr_info("%s fmt=0x%x, nDeviceID=%d \n", __func__, fmt, ak4376->nDeviceID);
...
static int ak4376_init_reg(struct snd_soc_codec *codec)
{
    ...
    pr_info("%s DeviceID=0x%x \n", __func__, DeviceID);

    switch (DeviceID >> 5) {
    case 0:
        ak4376->nDeviceID = 0;        //0:AK4375
        pr_info("%s AK4375 is connecting.\n", __func__);
        break;
    case 1:
        ak4376->nDeviceID = 1;        //1:AK4375A
        pr_info("%s AK4375A is connecting.\n", __func__);
        break;
    case 2:
        ak4376->nDeviceID = 2;        //2:AK4376
        printk("%s AK4376 is connecting.\n", __func__);
        break;
    default:
        ak4376->nDeviceID = 3;        //3:Other IC
        pr_info("%s This device are neither AK4375/A nor AK4376.\n", __func__);
    }
