...
static struct mbhc_usb_c_analog_switch oppo_usb_c_switch;
static enum wcd_mbhc_headset_type oppo_headset_type;
extern bool max20328_ADC_RES_OF(void);
extern int max20328_swap_mic_gnd(void);
#endif
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/25, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
		oppo_headset_type = MBHC_HEADSET_NONE;
#endif
#endif /* VENDOR_EDIT */
	} else {
		/*
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/25, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
			if (max20328_ADC_RES_OF()) {
				oppo_headset_type = MBHC_HEADSET_NORMAL;
				pr_debug("%s: MBHC_HEADSET_NORMAL\n",__func__);
			} else {
				oppo_headset_type = MBHC_HEADSET_SELFI_STICKER;
				pr_debug("%s: MBHC_HEADSET_SELFI_STICKER\n",__func__);
			}
#endif
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/06/29, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
		if ((oppo_headset_type == MBHC_HEADSET_SELFI_STICKER) && (btn_result == 7)) {
			pr_err("%s: ignoring lbp for special headset: %d. Swap audio switch.\n",
			 __func__, btn_result);
			max20328_swap_mic_gnd();
		} else {
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/06/29, Add for usb type-c audio */
		if (ret == 0) {
#else
#ifndef USB_SWITCH_MAX20328
		if (ret == 0) {
#else
		if ((ret == 0) && (oppo_headset_type == MBHC_HEADSET_SELFI_STICKER) && (mbhc->buttons_pressed == SND_JACK_BTN_3)) {
			pr_err("%s: ignoring lbp for special headset, swap audio switch\n",__func__);
			max20328_swap_mic_gnd();
		} else if (ret == 0) {
#endif /* USB_SWITCH_MAX20328 */
#endif /* VENDOR_EDIT */
...
#ifdef USB_SWITCH_MAX20328
	int val = -EINVAL;
	int mode_request = oppo_usb_c_switch.usbc_switch_mode_request;
	if (mode_request == oppo_usb_c_switch.usbc_switch_mode) {
		pr_info("%s: no need to change. mbhc->usbc_mode = %d,usbc_switch_mode = %d \n",
			__func__, mbhc->usbc_mode, oppo_usb_c_switch.usbc_switch_mode);
		return rc;
	}
	if (mode_request == AUDIO_SWITCH_MODE_AUDIO) {
		active = true;
	} else {
		active = false;
	}
	pr_info("%s:mode_request: %d, mbhc->usbc_mode %d, usbc_switch_mode %d \n",
		__func__, mode_request, mbhc->usbc_mode, oppo_usb_c_switch.usbc_switch_mode);
#endif
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/06/29, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
		WCD_MBHC_REG_READ(WCD_MBHC_MIC_CLAMP_CTL, val);
		if ((val != -EINVAL) && val) {
			WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_MIC_CLAMP_CTL, 0);
		}
		ak4376_HPLR_HZ(true);
		//wait for ak4367 pull-down resistor to be 200k
		msleep(10);

		mbhc->mbhc_cb->lock_sleep(mbhc, true);
		max20328_set_switch_mode(1);
		mbhc->mbhc_cb->lock_sleep(mbhc, false);

		ak4376_HPLR_HZ(false);
		msleep(10);
		if ((val != -EINVAL) && val) {
			WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_MIC_CLAMP_CTL, val);
		}
		oppo_usb_c_switch.usbc_switch_mode = AUDIO_SWITCH_MODE_AUDIO;
		oppo_headset_type = MBHC_HEADSET_NONE;
#endif
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/06/29, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
		if (mode_request == AUDIO_SWITCH_MODE_USB) {
			mbhc->mbhc_cb->lock_sleep(mbhc, true);
			rc = max20328_set_switch_mode(0);
			mbhc->mbhc_cb->lock_sleep(mbhc, false);
			oppo_usb_c_switch.usbc_switch_mode = AUDIO_SWITCH_MODE_USB;
		} else {
			mbhc->mbhc_cb->lock_sleep(mbhc, true);
			rc = max20328_set_switch_mode(0);
			mbhc->mbhc_cb->lock_sleep(mbhc, false);
			oppo_usb_c_switch.usbc_switch_mode = AUDIO_SWITCH_MODE_OFF;
		}
#endif /* USB_SWITCH_MAX20328 */
#endif /* VENDOR_EDIT */
...
#ifdef USB_SWITCH_MAX20328
static void wcd_mbhc_usbc_analog_gpio_work_fn(struct work_struct *work)
{
	struct wcd_mbhc *mbhc = oppo_usb_c_switch.mbhc;
	if (mbhc) {
		wcd_mbhc_usb_c_analog_setup_gpios(mbhc,
			oppo_usb_c_switch.usbc_switch_mode_request == AUDIO_SWITCH_MODE_AUDIO);
	}
}
#endif
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/03, Add for usb type-c audio */
	...
	wcd_mbhc_usb_c_analog_setup_gpios(mbhc,
			mbhc->usbc_mode != POWER_SUPPLY_TYPEC_NONE);
#else
	cancel_work_sync(&oppo_usb_c_switch.usbc_analog_gpio_work);
	if (oppo_usb_c_switch.usbc_switch_mode != oppo_usb_c_switch.usbc_switch_mode_request) {
		/* filter notifications received before */
		schedule_work(&oppo_usb_c_switch.usbc_analog_gpio_work);
	}
#endif
#endif /* VENDOR_EDIT */
...
#ifndef VENDOR_EDIT
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/05, Add for usb type-c audio */
...
#else
		oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_AUDIO;
		pr_info("%s: event change requested: AUDIO_SWITCH_MODE_AUDIO\n", __func__);
#endif
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/05, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
static int wcd_mbhc_usb_c_switch_init(struct wcd_mbhc *mbhc)
{
	int ret;
	union power_supply_propval mode;
	struct snd_soc_codec *codec = mbhc->codec;
	...
	pr_info("%s: POWER_SUPPLY_PROP_TYPEC_MODE: %d\n", __func__, mode.intval);
	switch (mode.intval) {
	case POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER:
		oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_AUDIO;
		break;
	case POWER_SUPPLY_TYPEC_NONE:
		oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_OFF;
		break;
	default:
		oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_USB;
		break;
	}
	schedule_work(&mbhc->usbc_analog_work);
	return ret;
...
static int usb_state_callback(struct notifier_block *self,
				 unsigned long event, void *data)
{
	int ret = 0;
	struct wcd_mbhc *mbhc = oppo_usb_c_switch.mbhc;
	if((event == POWER_SUPPLY_TYPEC_USBMODE)&&(mbhc)) {
		oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_USB;
		schedule_work(&mbhc->usbc_analog_work);
		pr_info("%s: USB type C state change requested: AUDIO_SWITCH_MODE_USB\n", __func__);
	} else if(event == POWER_SUPPLY_TYPEC_OFF) {
		oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_OFF;
		schedule_work(&mbhc->usbc_analog_work);
		pr_info("%s: USB type C state change requested: AUDIO_SWITCH_MODE_OFF\n", __func__);
	} else {
		ret = -1;
		pr_info("%s: USB type C state change requested: ERR UNKNOWN\n", __func__);
	}
	return ret;
}
#endif
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/03, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
	oppo_usb_c_switch.mbhc = mbhc;
	oppo_headset_type = MBHC_HEADSET_NONE;
	INIT_WORK(&oppo_usb_c_switch.usbc_analog_gpio_work, wcd_mbhc_usbc_analog_gpio_work_fn);
	oppo_usb_c_switch.usbc_switch_mode = AUDIO_SWITCH_MODE_OFF;
	oppo_usb_c_switch.usbc_switch_mode_request = AUDIO_SWITCH_MODE_USB;
#endif
#endif /* VENDOR_EDIT */
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/05, Add for usb type-c audio */
#ifdef USB_SWITCH_MAX20328
	oppo_usb_c_switch.usb_state_nb.notifier_call = usb_state_callback;
	oppo_usb_c_switch.usb_state_nb.priority = 0;
	ret = msm_usb_register_client(&oppo_usb_c_switch.usb_state_nb);
	if (ret) {
		dev_err(codec->dev, "%s: usb_state_callback registration failed\n",
			__func__);
		goto err;
	}
#endif
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/09/03, Add for usb type-c audio */
	...
	ret = wcd_mbhc_usb_c_event_changed(&mbhc->psy_nb,
					   ...
					   mbhc->usb_psy);
#else
	ret = wcd_mbhc_usb_c_switch_init(mbhc);
#endif
#endif /* VENDOR_EDIT */
