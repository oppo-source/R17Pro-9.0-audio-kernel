...
static const char *const oppo_typec_switch_mic_state_text[] = {"OPEN", "SBU1", "SBU2"};
static SOC_ENUM_SINGLE_EXT_DECL(oppo_typec_switch_mic_state, oppo_typec_switch_mic_state_text);
extern int max20328_get_sbu_cfg_state(void);

static int typec_switch_state_get(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = max20328_get_sbu_cfg_state();

	return 0;
}

static int typec_switch_state_put(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	pr_err("%s: do nothing. \n", __func__);
...
/* Huiqun.Han@PSW.MM.AudioDriver.Machine, 2018/07/21, Add for usb type-c audio engineer test */
#ifdef USB_SWITCH_MAX20328
	SOC_ENUM_EXT("Type-C Switch Mic State", oppo_typec_switch_mic_state,
			typec_switch_state_get,
			typec_switch_state_put),
#endif
#endif /* VENDOR_EDIT */
};
...
static int msm_prepare_us_euro(struct snd_soc_card *card)
{
	...
	#ifdef USB_SWITCH_MAX20328
	ret = 0;
	pr_info("%s: using external type-c audio switch.\n", __func__);
	#endif
	#endif /* VENDOR_EDIT */
	return ret;
}
...
extern void iacore_notify_fw_download(void);
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (pdata->snd_card_val == INT_SND_CARD) {
		...
		#ifdef VENDOR_EDIT
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec.1263116, 2018/07/13, Add for smartmic*/
		pdata->dmic34_gpio_p = of_parse_phandle(pdev->dev.of_node,
					"qcom,cdc-dmic34-gpios", 0);
		#endif /* VENDOR_EDIT */
	...
	#ifdef VENDOR_EDIT
	/* Jianfeng.Qiu@PSW.MM.AudioDriver.Codec.1263116, 2018/05/15,
	 * Add for smart mic, defaut set dmic pdm to high-z state
	 */
	if ((pdata->snd_card_val == INT_SND_CARD)) {
		pr_info("%s: set de-activated %s", __func__, "dmic34_gpio");
		if (msm_cdc_pinctrl_select_sleep_state(pdata->dmic34_gpio_p)) {
			pr_err("%s: cannot de-activated %s", __func__, "dmic34_gpio");
		}
		pdata->adsp_restart_flag = false;
	}

	#if defined(CONFIG_SND_SOC_IA6XX)
	iacore_notify_fw_download();
	#endif /* CONFIG_SND_SOC_IA6XX */
	#endif /* VENDOR_EDIT */
