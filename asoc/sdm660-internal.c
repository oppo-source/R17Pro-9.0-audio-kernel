...
#if defined(CONFIG_SND_SOC_IA6XX)
extern int iacore_set_spl_pdm_mode_sts(bool value);
#endif /* CONFIG_SND_SOC_IA6XX */
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec, 2018/05/31, Add for smart mic*/
static const char *const knowles_mic_text[] = {"Off", "On"};
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec, 2018/05/31, Add for smart mic*/
static SOC_ENUM_SINGLE_EXT_DECL(knowles_mic_enum, knowles_mic_text);
static int knowles_mic_status = 0;
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* Jianfeng.Qiu@PSW.MM.AudioDriver.Codec.1263116, 2018/05/15, Add for smart mic */
static int adnc_spl_mode_event(struct snd_soc_dapm_widget *w,
			       struct snd_kcontrol *kcontrol, int event);
#endif /* VENDOR_EDIT */
...
static const struct snd_soc_dapm_widget msm_int_dapm_widgets[] = {
	...
	#ifdef VENDOR_EDIT
	/* Jianfeng.Qiu@PSW.MM.AudioDriver.Codec.1263116, 2018/05/15, Add for smart mic */
	SND_SOC_DAPM_MICBIAS_E("ADNC SPL Mode", 0, 0, 0,
		adnc_spl_mode_event, SND_SOC_DAPM_PRE_PMU |
...
#ifdef VENDOR_EDIT
/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec, 2018/05/31, Add for smart mic*/
static int knowles_mic_get_status(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s knowles_mic_status:%d \n", __func__, knowles_mic_status);
	ucontrol->value.integer.value[0] = knowles_mic_status;

	return 0;
}

static int knowles_mic_put_status(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s status:%ld\n", __func__, ucontrol->value.integer.value[0]);

	knowles_mic_status = ucontrol->value.integer.value[0];
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec, 2018/05/31, Add for smart mic*/
	SOC_ENUM_EXT("Knowles_Mic_Switch", knowles_mic_enum,
			knowles_mic_get_status, knowles_mic_put_status),
	#endif /* VENDOR_EDIT */
};
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_info("%s: event: %d, adsp_restart_flag: %d.\n",
		__func__, event , pdata->adsp_restart_flag);

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		/* Make sure dmic gpio state same with ap after adsp restart.
		 * current_value: ture is active state, false is sleep state.
		 * 1 Get current state
		 * 2 Set converse state
		 * 3 Set current state
		 */
		if (pdata->adsp_restart_flag) {
			current_value = msm_cdc_pinctrl_get_state(
				pdata->dmic34_gpio_p);

			pdata->adsp_restart_flag = false;
			...
			if (ret < 0) {
				pr_err("%s: set dmic gpio state failed.\n", __func__);
			} else {
				pr_debug("%s: %s set %s%s state success.\n",
					__func__, "dmic34_gpio_p",
		...
		#if defined(CONFIG_SND_SOC_IA6XX)
		iacore_set_spl_pdm_mode_sts(true);
		#endif /* CONFIG_SND_SOC_IA6XX */
		ret = msm_cdc_pinctrl_select_active_state(
			pdata->dmic34_gpio_p);
		if (ret < 0) {
			pr_err("%s: %s cannot be activated.\n",
					__func__, "dmic34_gpio_p");
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		ret = msm_cdc_pinctrl_select_sleep_state(
			pdata->dmic34_gpio_p);
		if (ret < 0) {
			pr_err("%s: %s cannot be de-activated.\n",
					__func__, "dmic34_gpio_p");
		}
		msleep(3);
		#if defined(CONFIG_SND_SOC_IA6XX)
		iacore_set_spl_pdm_mode_sts(false);
		#endif /* CONFIG_SND_SOC_IA6XX */
		break;
	default:
		return -EINVAL;
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec, 2018/06/11,
	 *Add for fix voice call TX mute when AP sleep.
	 */
...
#ifdef VENDOR_EDIT
/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec.1263116, 2018/01/08, Add for smart mic*/
static struct snd_soc_dai_link audience_be_dai_links[] = {
	{
		.name = LPASS_BE_PRI_MI2S_TX,
		.stream_name = "Primary MI2S Capture",
		.cpu_dai_name = "msm-dai-q6-mi2s.0",
		.platform_name = "msm-pcm-routing",
		.codec_name = "ia6xx-codec",
		.codec_dai_name = "ia6xx-mic",
		.no_pcm = 1,
		.dpcm_capture = 1,
					...
					struct device *dev, int snd_card_val)
{
	...
	const char *oppo_smartmic_type = "oppo,smartmic-vendor";
	...
	if (of_property_read_bool(dev->of_node,
		...
		if (!of_property_read_string(dev->of_node, oppo_smartmic_type,
				&product_name)) {
			pr_info("%s: custom smartmic product %s\n", __func__, product_name);
			for (i = 0; i < ARRAY_SIZE(msm_mi2s_be_dai_links); i++) {
				temp_link = &msm_mi2s_be_dai_links[i];
				if (temp_link->id == MSM_BACKEND_DAI_PRI_MI2S_TX) {
					if (!strcmp(product_name, "audience")
						&& soc_find_component(NULL, audience_be_dai_links[0].codec_name)) {
						pr_info("%s: use audience smartmic dailink replace\n", __func__);
						memcpy(temp_link, &audience_be_dai_links[0],
							sizeof(audience_be_dai_links[0]));
						break;
					}
				}
			}
